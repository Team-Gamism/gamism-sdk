= ASP.NET Core 서버 가이드
:toc:
:toc-title: 목차
:sectanchors:
:sectlinks:
:sectnums:

`Gamism.SDK.Core`와 `Gamism.SDK.Extensions.AspNetCore` 패키지 사용 가이드입니다.

== 설치

[source,xml]
----
<PackageReference Include="Gamism.SDK.Core" Version="x.x.x" />
<PackageReference Include="Gamism.SDK.Extensions.AspNetCore" Version="x.x.x" />
----

== 등록

`Program.cs`에 두 줄만 추가하면 모든 기능이 활성화됩니다. 각 기능은 기본적으로 활성화(`Enabled = true`)되어 있습니다.

[source,csharp]
----
builder.Services.AddGamismSdk(options =>
{
    options.ConfigureSwagger(swagger =>
    {
        swagger.Title = "My API";
        swagger.PathsToMatch = ["/api/**"];
    });

    options.ConfigureLogging(logging =>
    {
        logging.NotLoggingUrls = ["/swagger/**", "/health"];
    });
});

app.UseGamismSdk();
----

== CommonApiResponse<T>

서버와 클라이언트가 공유하는 공통 API 응답 포맷입니다.

=== 응답 구조

[source,json]
----
{
  "status": "OK",
  "code": 200,
  "message": "성공",
  "data": <페이로드>
}
----

=== 팩토리 메서드

[cols="2,1,2", options="header"]
|===
| 메서드 | HTTP Status | 설명

| `CommonApiResponse.Success(message)`
| 200 OK
| data 없음

| `CommonApiResponse.Success(message, data)`
| 200 OK
| data 포함

| `CommonApiResponse.Created(message)`
| 201 Created
| data 없음

| `CommonApiResponse.Created(message, data)`
| 201 Created
| data 포함

| `CommonApiResponse.Error(message, status)`
| 지정한 status
| data 없음
|===

== 로깅 필터 (options.Logging)

HTTP 요청과 응답을 자동으로 로깅합니다.

=== 로그 포맷

요청:

[source]
----
[Request] GET /api/users?page=1
----

응답:

[source]
----
[Response] 200 (42ms)
----

=== 설정 옵션

[cols="2,1,3", options="header"]
|===
| 옵션 | 기본값 | 설명

| `Enabled`
| `true`
| 로깅 필터 활성화 여부

| `NotLoggingUrls`
| `[]`
| 로깅에서 제외할 URL 패턴 목록. `**` 와일드카드 지원
|===

[source,csharp]
----
options.ConfigureLogging(logging =>
{
    logging.NotLoggingUrls = ["/health", "/swagger/**"];
});
----

== 응답 래퍼 (options.Response)

컨트롤러의 모든 반환값을 `CommonApiResponse<T>` 형식으로 자동 래핑합니다. `IAsyncResultFilter`로 구현되어 있어 별도 코드 없이 동작합니다.

=== 케이스별 동작

[cols="2,1,2", options="header"]
|===
| 반환값 | HTTP Status | 동작

| 일반 객체
| 200 OK
| `CommonApiResponse<T>` 로 자동 래핑

| `null`
| 204 No Content
| 빈 응답

| `ICommonApiResponse` 구현체
| 응답 내 `code` 값
| 그대로 통과 (이중 래핑 없음)
|===

=== 컨트롤러 예시

[source,csharp]
----
[ApiController]
public class UserController : ControllerBase
{
    [HttpGet("/users")]
    public List<User> GetUsers()
    {
        // 자동으로 CommonApiResponse<List<User>>로 래핑됨
        return _userService.GetAll();
    }

    [HttpPost("/users")]
    public CommonApiResponse<User> CreateUser(CreateUserRequest request)
    {
        // 직접 반환하면 이중 래핑 없이 그대로 응답
        return CommonApiResponse.Created("생성 완료", _userService.Create(request));
    }
}
----

=== 설정 옵션

[cols="2,1,3", options="header"]
|===
| 옵션 | 기본값 | 설명

| `Enabled`
| `true`
| 응답 래퍼 활성화 여부

| `NotWrappingUrls`
| `[]`
| 래핑에서 제외할 URL 패턴 목록. `**` 와일드카드 지원
|===

[source,csharp]
----
options.ConfigureResponse(response =>
{
    response.NotWrappingUrls = ["/swagger/**", "/health"];
});
----

== 예외 처리 (options.Exception)

`ExpectedException`을 던지면 `GlobalExceptionHandler`가 자동으로 처리하여 `CommonApiResponse` 형식으로 응답합니다. 그 외 예외는 500으로 응답하고 스택 트레이스를 `ILogger`에 기록합니다.

=== 기본 사용법

[source,csharp]
----
// 메시지 직접 지정
throw new ExpectedException(HttpStatusCode.NotFound, "유저를 찾을 수 없습니다");

// status 기본 메시지 사용
throw new ExpectedException(HttpStatusCode.Forbidden);
----

=== 응답 예시

[source,json]
----
{
  "status": "NOT_FOUND",
  "code": 404,
  "message": "유저를 찾을 수 없습니다"
}
----

=== 내장 예외 클래스

자주 쓰는 상태코드는 전용 클래스를 제공합니다.

[cols="2,1", options="header"]
|===
| 클래스 | HTTP Status

| `BadRequestException`
| 400

| `UnauthorizedException`
| 401

| `ForbiddenException`
| 403

| `NotFoundException`
| 404

| `ConflictException`
| 409
|===

[source,csharp]
----
throw new NotFoundException("유저를 찾을 수 없습니다");
throw new ForbiddenException("접근 권한이 없습니다");
----

=== 성능

`ExpectedException`은 스택 트레이스를 수집하지 않아 일반 예외보다 생성 비용이 낮습니다. 비즈니스 로직 오류 처리에 적합합니다.

=== 설정 옵션

[cols="2,1,3", options="header"]
|===
| 옵션 | 기본값 | 설명

| `Enabled`
| `true`
| 예외 핸들러 활성화 여부
|===

== Swagger (options.Swagger)

Swashbuckle 기반의 Swagger UI를 자동으로 구성합니다. 응답 스키마는 `CommonApiResponse` 래핑 구조를 반영합니다.

=== 설정 옵션

[cols="2,1,3", options="header"]
|===
| 옵션 | 기본값 | 설명

| `Enabled`
| `true`
| Swagger 활성화 여부

| `Title`
| `"API Documentation"`
| API 문서 제목

| `Description`
| `"API Documentation"`
| API 문서 설명

| `Version`
| `"v1"`
| API 버전

| `Group`
| `"default"`
| API 그룹명

| `PathsToMatch`
| `["/**"]`
| 문서화할 경로 패턴 목록. `**` 와일드카드 지원
|===

[source,csharp]
----
options.ConfigureSwagger(swagger =>
{
    swagger.Title = "My API";
    swagger.Version = "v1";
    swagger.PathsToMatch = ["/api/**"];
});
----

== 전체 설정 레퍼런스

[source,csharp]
----
builder.Services.AddGamismSdk(options =>
{
    options.ConfigureException(exception =>
    {
        exception.Enabled = true;
    });

    options.ConfigureResponse(response =>
    {
        response.Enabled = true;
        response.NotWrappingUrls = ["/swagger/**", "/health"];
    });

    options.ConfigureLogging(logging =>
    {
        logging.Enabled = true;
        logging.NotLoggingUrls = ["/swagger/**", "/health"];
    });

    options.ConfigureSwagger(swagger =>
    {
        swagger.Enabled = true;
        swagger.Title = "My API";
        swagger.Description = "My API Description";
        swagger.Version = "v1";
        swagger.Group = "default";
        swagger.PathsToMatch = ["/api/**"];
    });
});

app.UseGamismSdk();
----
